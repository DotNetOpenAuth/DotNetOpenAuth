					...
					try {
						foreach (var returnUrl in Realm.Discover(false)) {
							...
							if (discoveredReturnToUrl.DomainWildcard) {
								TraceUtil.Logger.WarnFormat("Realm {0} contained return_to URL {1} which contains a wildcard, which is not allowed.",
						...
						TraceUtil.Logger.InfoFormat("Relying party discovery at URL {0} failed.  {1}",
								Realm, ex);
