				...
				foreach (var uri in service.UriElements) {
					...
					if (userSuppliedOrClaimedIdentifier is XriIdentifier) {
						if (service.Xrd.CanonicalID == null) {
							TraceUtil.Logger.WarnFormat(Strings.MissingCanonicalIDElement, userSuppliedOrClaimedIdentifier);
							return null;
						}
						...
						if (performCIDVerification && userSuppliedOrClaimedIdentifier != service.Xrd.CanonicalID) {
							TraceUtil.Logger.InfoFormat("Performing XRI CanonicalID verification on user supplied identifier {0}, canonical id {1}.", userSuppliedOrClaimedIdentifier, service.Xrd.CanonicalID);
